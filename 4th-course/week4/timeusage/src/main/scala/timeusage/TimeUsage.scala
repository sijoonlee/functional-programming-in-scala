...
//    val grouped:Map[String, List[(String, Column)]] = columnNames
//      .map(name => {
//        if (primaryNeeds.exists(name.startsWith)) ("primary", col(name))
//        else if (workingActivities.exists(name.startsWith)) ("working", col(name))
//        else if (otherActivities.exists(name.startsWith)) ("other", col(name))
//        else ("n/a", col("n/a"))
//      })
//      .groupBy(_._1)
//
//    (grouped("primary").map(_._2), grouped("work").map(_._2), grouped("other").map(_._2))

    val grouped = columnNames.groupBy(c =>
      if(primaryNeeds.exists(c.startsWith)) "primary"
      else if(workingActivities.exists(c.startsWith)) "work"
      else if(otherActivities.exists(c.startsWith)) "other"
      else "unused"
