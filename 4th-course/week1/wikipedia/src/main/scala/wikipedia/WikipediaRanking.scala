...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  val wikiRdd: RDD[WikipediaArticle] = sc.parallelize(WikipediaData.lines.map(WikipediaData.parse), 2).persist()
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] =
    rdd.flatMap( a => langs
                      .filter(a.mentionsLanguage)
                      .map((_, a)) // (lang, article)
    ).groupByKey // (lang, Iterable[article]) )
