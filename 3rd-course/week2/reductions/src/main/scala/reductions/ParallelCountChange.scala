...
object ParallelCountChange extends ParallelCountChangeInterface {
  ...
  def countChange(money: Int, coins: List[Int]): Int = money match {
    case 0 => 1
    case i if (i > 0) => {
      if (!coins.isEmpty) countChange(money - coins.head, coins) + countChange(money, coins.tail)
      else 0
    }
    case _ => 0
  }
  ...
  def moneyThreshold(startingMoney: Int): Threshold =
    ...
    // To avoid this problem, be sure to always do the multiplication of startingMoney by 2 before doing the division by 3.
