...
object Conc {
  ...
  private def concat[T](xs: Conc[T], ys: Conc[T]): Conc[T] = {
    ...
    if (diff >= -1 && diff <= 1) new <>(xs, ys)
      ...
      // ys.level <= xs.level - 2
      if (xs.left.level >= xs.right.level) { // xs.left must have d - 1 level (xs has d level)
        val nr = concat(xs.right, ys)
        new <>(xs.left, nr)
      } else { // xs.left: d-2, xs.right: d-1
        val nrr = concat(xs.right.right, ys) // xs.right.right has d - 2 or d - 3
        if (nrr.level == xs.level - 3) {
          // xs.right.right changed with no need of re-balance
          //                               d
          //       d-2                                 d-1
          //                                  d-2                 d-3
          // xs.left(d-2)          xs.right.left(d-2)       xs.right.right(d-3)    ys (concat with sub-tree of xs.right.right)
          val nl = xs.left
          ...
          //                               d
          //            d-1                                         d-1 or d-2
          // xs.left(d-2)   xs.right.left(d-2 or d-3)      xs.right.right(d-2 or d-3)    ys(d-2 or d-3)
  ...
  def appendTop[T](xs: Conc[T], ys: Leaf[T]): Conc[T] = (xs: @unchecked) match {
    ...
    case _ <> _ => new Append(xs, ys) // if it is inner node
    case Empty => ys
  ...
  @tailrec private def append[T](xs: Append[T], ys: Conc[T]): Conc[T] = {
    if (xs.right.level > ys.level) new Append(xs, ys)
      ...
      xs.left match {
        case ws @ Append(_, _) => append(ws, zs) // recursively happen till the bottom, which is not Append(_,_) but Leaf
